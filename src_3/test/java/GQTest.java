...
public class GQTest {
    ...
    void testDumpList() {
    	stringQueue.dequeue();
    	stringQueue.setHead(new GenericQueue.Node<>("a"));
    	stringQueue.add("b");
    	stringQueue.add("c");

        ArrayList<String> dumped = stringQueue.dumpList(); 

        assertTrue(dumped.contains("a"), "dumpList() should contain 'a'");
        assertTrue(dumped.contains("b"), "dumpList() should contain 'b'");
        assertTrue(dumped.contains("c"), "dumpList() should contain 'c'");
    }
